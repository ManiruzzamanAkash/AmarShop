...
use App\Notifications\SendProductRequest;
...
class AdminProductController extends Controller
{
  ...
  {
    $products = Product::orderBy('id', 'desc')->where('publish_status', 1)->paginate(2);
    return view('pages.product.index')->withProducts($products);
  }

  public function admin_request()
  {
    $products = Productrequest::orderBy('id', 'desc')->paginate(2);
    return view('admin.pages.product.request')->withProducts($products);
  }
  ...
  public function store(Request $request)
  {
    ...
    if ((Auth::user()->trust_point >= 100) || (Auth::user()->is_admin != 0)) { //This is a verified customer who is trusted by 10 customers
  ...
  public function show($slug)
  {
    ...
    $category_id = $product->ctegory_id;
  ...
  public function changeProductStatus($id, $post)
  {
    if ($post == "true") {
      ...
      if ($publish_status == 1) {
        ...
        $product->publish_status = 1;
        $product->save();
        //Send Notification to the users who wants this
        //Email to those users who have posted this category product 
        $category_id = $product->category_id;

        $products = Product::where('category_id', $category_id)->get();

        foreach ($products as $product_by) {
          $user = $product_by->user;
          //send an email to the user
          $user->notify(new SendProductRequest($user, $product_request));
        }
      }
